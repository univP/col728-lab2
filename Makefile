...
clean::
	rm -f c.tab.cpp c.tab.hpp c.lex.cpp cc c.output output.o semant.tree semant1.tree dout a.out bitcode.bc pass.so cse-pass.o code.bc code.ll

code: pass.so bitcode.bc
	opt -load ./pass.so -cse < bitcode.bc > code.bc

pass.so: cse-pass.cpp
